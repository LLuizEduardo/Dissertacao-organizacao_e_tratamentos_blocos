...
# set folder ------------------------------------------------------------------
  ...
  lista <- 
  financial_market3 <- list('ISEQ','AEX','poland_WIG20','IBEX')
            substitute(
          financial_market3
          )->nomeBloco
...
x<-getSymbols(paste0('^',lista[[4]]), src = 'yahoo', env = NULL, from = '1990-01-01', periodicity = 'monthly')
...
mf<-read.csv(sep = ";","C:/Users/luiz.eduardo/OneDrive/Documentos/Dissertação/base de dados - tratamento/-V1/v1 Selecao/series csv mercado financeiro/poland_WIG20.csv")
names(mf) <- sub("poland_", "", names(mf))
c<-merge(c,mf, by = c('date'), all = TRUE)  
rm(mf)
